  ...
  const handleApprove = async () => {
    ...
    try {
      const res = await axios.post(
        "https://grokart-2.onrender.com/api/v1/delivery/approve",
        { email },
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex justify-center items-center px-4">
      <div className="max-w-xl w-full bg-white p-8 rounded-2xl shadow-xl">
        <h2 className="text-3xl font-bold mb-6 text-gray-800">Approve Delivery Partner</h2>
        <div className="mb-4">
          <label className="block text-sm font-medium text-gray-600 mb-1">Delivery Partner Email</label>
          <input
            type="email"
            value={email}
            onChange={(e) => setEmail(e.target.value)}
            className="w-full px-4 py-2 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500"
            placeholder="Enter delivery partner's email"
          />
        </div>
