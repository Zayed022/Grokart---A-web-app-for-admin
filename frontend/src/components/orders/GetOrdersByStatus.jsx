...
const OrderManagement = () => {
  ...
  const handleUpdateStatus = async (orderId, newStatus) => {
    try {
      ...
      // ✅ Notify user after successful status update
      const updatedOrder = res.data?.order;
      if (updatedOrder?.customerId?._id) {
  ...
  // 🔔 Helper: Notify specific customer
    ...
    try {
      ...
      switch (status) {
        case "Placed":
          ...
          body = "Your order has been placed. We'll confirm it shortly.";
          break;
        case "Assigned":
        ...
        case "Out for Delivery":
          ...
          body = "Your order is on the way. Expect delivery soon!";
          break;
        case "Delivered":
    ...
    return (
      <div className="space-y-1">
        ...
        <small className="text-xs text-gray-500 leading-snug block">
          {d.houseNumber}, {d.floor}, {d.buildingName}, {d.landmark}, {d.city},{" "}
          {d.state} - {d.pincode} <br />
                  ...
                  >
                    ...
                    <td className="px-4 py-2">
                      {order.status === "Placed" ? (
                        <select
                          value={
                            selectedAssignments[order._id]?.partnerId || ""
                          }
                          onChange={(e) =>
                            setSelectedAssignments((prev) => ({
                              ...prev,
